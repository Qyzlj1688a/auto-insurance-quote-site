...
### 0. 環境変数ファイル（.env）
> **レビュー担当者向け**: 動作確認は `http://localhost:5173` で行ってください。
> 同梱 `.env` はこのURL向けに設定済みです。IPアドレス（`192.168.x.x` や公開IP等）で
> アクセスする場合は、ブラウザのアドレスバーに表示されるURLを `CORS_ALLOWED_ORIGINS` に
> 追加し、`docker compose down` → `docker compose up -d` でコンテナを再作成してください。
> （`restart` では環境変数は反映されません。ログイン時に「システムエラー」が出る場合はこの設定を確認してください。）

本リポジトリには、レビュー担当者が追加作業なしで起動できるよう、
**課題用の `.env` を同梱**しています。`docker compose up` 実行時に自動読み込みされます。
